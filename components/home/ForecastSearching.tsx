...
export function ForecastSearching({ duration, usage }: ForecastSearchingProps) {
  ...
  return (
    <div className="relative flex flex-col gap-4 overflow-hidden">
      ...
      <div aria-hidden className="rounded-xl bg-ink-900 px-5 py-4">
        <div className="flex items-center gap-3">
          <span className="size-8 shrink-0 rounded-full bg-ink-700" />
          <div className="flex-1">
            <span className="block h-3 w-full rounded-full bg-ink-700" />
            <span className="mt-2 block h-3 w-2/3 rounded-full bg-ink-700" />
          </div>
        </div>
      </div>
        ...
        <ul className="mt-3 grid list-none gap-3 sm:grid-cols-3">
          {steps.map((label, index) => (
            <li
              key={label}
              className={cn(
                "overflow-hidden rounded-lg border border-neutral-200 bg-white transition-opacity dur-slow ease-out",
